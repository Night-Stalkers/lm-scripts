    ...
    def think (self, dt):
        ...
        if player: # If so...
            if self.dead or not player.world_object: # Releases if dead or if no longer a valid object
                ...
                if tip.z >= 63: # We are hitting the water and dragging them down.
                    player.kraken_kill ()
        ...
        if not self.parent.xrotation:
            if abs (self.pos.x + offset-self.origin_x) < 2:
                ...
                for x, y, z in self.blocks:
                    self.protocol.build_block(x, y, z, KRAKEN_BLACK, force = True)
                self.blocks = set()
            self.pos.x += offset
            self.create_instant ()
        else:
    ...
    class BossConnection(connection):
        ...
        def on_position_update(self):
            ...
            if self.world_object:
                if (not self.world_object.dead and not self.grabbed_by and not self.trapped):
                    for player in self.protocol.players.values():
                        if player.world_object:
