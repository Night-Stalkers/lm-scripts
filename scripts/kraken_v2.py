...
if NEW_WEAPONS:
    ...
    Rifle.stock = 25 # Rifle ammo: 10 in the initial magazine + 20 reserved
    SMG.stock = 60 # SMG ammo: 30 in initial magazine + 60 reserved
...
# Shapes are pretty cool! These are used to draw the kraken. 
def cube (s):
    s0, s1 = -s / 2 + 1, s / 2 + 1
...
def disc (rr, x = 0, y = 0, min_rr = None):
    for u, v in plane (rr):
        d = u * u + v * v
        if not (d > rr or (min_rr and d < min_rr)): # Removes the original continue statement
            yield x + u, y + v
def sphere (r, x = 0, y = 0, z = 0, min_r = None):
    ...
    for w, v, u in cube(r):
        d = u * u + v * v + w * w
        if not (d > rr or (min_r and d < min_rr)): # Removes the original continue statement
            yield x + u, y + v, z + w
...
def block_location_centered (x, y, z, i, j, k, s):
    return not (x < i - s or x > i + s or y < j - s or y > j + s or z < k - s or z > k + s)
...
# Returns TRUE if the player is alive and not godded.
def is_valid_enemy(player):
    if not player.world_object:
        return False
    return not (player.world_object.dead or player.grabbed_by or player.trapped or player.regenerating or player.god)

# Animates moving parts of the kraken such as the eyes and tentacles
class Animated:
    blocks_per_cycle = 5
    build_interval = 0.005
    build_queue = None
    build_loop = None
    blocks = None
    ...
    def fracture(self, x, y, z):
        ...
        while self.sections:
            ...
            if breakpoint:
                break
            i, j, k = pos.get()
            breakpoint = block_location_centered (x, y, z, i, j, k, radius)
        if self.sections:
            self.sections.pop()
...
def start_kraken(protocol, x, y, hardcore = False, finally_call = None):
    ...
    def falling_blocks_start():
        protocol.send_chat('Watch yer heads, duck and cover me hearties!', global_message = True)
    ...
    def progress_hardcore(caller = None):
        ...
        if boss.phase == 1:
            ...
            boss.dead = False
            boss.on_last_tentacle_death = round_end
            spawn_tentacles(5, respawn = False, fast = True, arena = True)
        elif boss.phase == 6:
            red_sky()
            boss.on_last_tentacle_death = None
            boss.on_death = victory
...
def apply_script(protocol, connection, config):
	...
	new_spawn_pos=False
		...
		if not player.world_object:
		    continue
                ...
                s = size + 3.0
                if block_location (i, j, k, x - 1.5, y - 1.5, z - 5.0, s, s, 6.0):
	...
	def on_refill(self):
		...
		if self.player_id!=killer.player_id:
            ...
            if self.protocol.boss:
                ...
                if (self.protocol.boss and not self.protocol.boss.dead and
                    ...
                    self.send_chat('You\'re down! Yell to your mateys for help!')
		...
		self.send_chat ('Arrrrrrrrrrrr! This cannon reloads very slowly!')
	...
	def free_from_kraken(self):
		...
		self.send_chat ('The kraken spit you out! Help your mateys!')
