...
from piqueserver.config import config as conf

HIDE_COORD = (0, 0, 0)


TDM_CONFIG = conf.section('tdm')
KILL_LIMIT_OPT = TDM_CONFIG.option('kill_limit', 100)
INTEL_POINTS_OPT = TDM_CONFIG.option('intel_points', 10)
REMOVE_INTEL_OPT = TDM_CONFIG.option('remove_intel', False)
...
def apply_script(protocol, connection, config):
    ...
    class TDMProtocol(protocol):
        ...
        protocol.kill_limit = KILL_LIMIT_OPT.get()
        protocol.intel_points = INTEL_POINTS_OPT.get()
        protocol.remove_intel = REMOVE_INTEL_OPT.get()
